...
def evaluteTop1_5(classfication, lines, metrics_out_path):
    ...
    # outscore=np.concatenate(s)
    # print(outscore.shape)
    # pre_label=pre_
    score = pd.DataFrame(outscore)  # 将classes每个种类的得分写入表格
    real_label = pd.Series(real_label)
    pre_label = pd.Series(pre_label)
    final_0 = pd.concat([real_label, pre_label], axis=1, ignore_index=True)  # 将真实标签和预测标签合并
    final_1 = pd.concat([final_0, score], axis=1, ignore_index=True)  # 将标签和得分合并

    final_1.rename(columns={0: 'real_label', 1: 'pre_label', 2: 0, 3: 1, 4: 2, 5: 3}, inplace=True)
    final_1.to_csv('./site.csv', index=False)
